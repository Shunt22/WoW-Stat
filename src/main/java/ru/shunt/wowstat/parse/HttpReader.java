...
public class HttpReader {
    ...
    private TalentsRepository talRep;
    private SpecRepository specRep;
    private ParsingRepository parsingRepository;
    private RacesRepository rRep;

    public HttpReader(Brackets bracket, String host, TalentsRepository cRep, SpecRepository sRep, ParsingRepository pRep, RacesRepository rRep) {
    ...
    @Deprecated
    private String checkForJsonOld(String url) throws IOException {
    ...
    public void readCharacters(List<WowCharacter> charactersList) throws IOException {
            ...
            try {
                ...
                if (characterJson != null) {
                    try {
                        jsonWorker.parseCharacter(character, characterJson, talRep, bracket.toString()); // can
                        // throw
                        // NPE
                    } catch (NullPointerException e) {
                        /*
                         * Something wrong with this character
						 * Usually it happens when character was transfered to other realm
						 */
                    }
                ...
                * Adding spec info
                */
                Specialization spec = specRep.findBySpecIdAndBracketName(character.getSpecId(), bracket.toString());
                if (spec == null) {
                    specRep.save((new Specialization(character.getSpecId(), 1, bracket.toString())));
                } else {
				 ...
				 * Adding race info
				 */
                Races race=rRep.findByRaceIdAndBracketName(character.getRaceId(),bracket.toString());
                if(race==null){
                    rRep.save(new Races(character.getRaceId(),1,bracket.toString()));
                }else{
